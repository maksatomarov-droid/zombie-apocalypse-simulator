...
nav_tabs = {
    "RU": [
        ...
        ("🧟", "6. Если ты зомби", "Юмористический раздел — как эффективно устроить апокалипсис. Adversarial Attack анализ 😄"),
        ("🧠", "7. ИИ-аналитик", "Задай вопрос ИИ-эпидемиологу: о пандемиях, вакцинах, защите от вирусов"),
        ("📊", "8. ML-модель", "Random Forest классификатор — предсказывает выживаемость на основе машинного обучения"),
        ("📖", "9. О проекте", "Описание проекта, SIR-модель, стек технологий, глоссарий терминов"),
    ],
    "EN": [
        ...
        ("🧟", "6. If You're a Zombie", "Humorous section — how to efficiently cause an apocalypse. Adversarial Attack analysis"),
        ("🧠", "7. AI Analyst", "Ask an AI epidemiologist: about pandemics, vaccines, virus protection"),
        ("📊", "8. ML Model", "Random Forest classifier — predicts survival based on machine learning"),
        ("📖", "9. About", "Project description, SIR model, tech stack, glossary of terms"),
    ],
    "KZ": [
        ...
        ("🧟", "6. Егер зомби болсаң", "Юмористикалық бөлім — апокалипсисті тиімді ұйымдастыру жолы 😄"),
        ("🧠", "7. ЖИ-аналитик", "ЖИ-эпидемиологқа сұрақ қой: пандемия, вакцина, вирустардан қорғану"),
        ("📊", "8. МЛ-үлгі", "Random Forest классификаторы — машиналық оқыту негізінде тіршілік ету мүмкіндігін болжайды"),
        ("📖", "9. Жоба туралы", "Жоба сипаттамасы, SIR моделі, технологиялар стегі, терминдер сөздігі"),
    ],
}
...
with st.expander(nav_titles.get(lang, nav_titles["RU"]), expanded=False):
    ...
    for i, (emoji, title, desc) in enumerate(tabs_info):
        with cols[i % 3]:
            st.markdown(f"""
            <div style="background:#111;border:1px solid #222;border-left:3px solid #ff3333;border-radius:8px;padding:.9rem 1rem;margin-bottom:.75rem;height:120px;overflow:hidden;">
                <div style="font-size:1.3rem;margin-bottom:.3rem;">{emoji}</div>
                <div style="color:#ff6666;font-weight:600;font-size:.9rem;margin-bottom:.3rem;">{title}</div>
                <div style="color:#666;font-size:.78rem;line-height:1.4;">{desc}</div>
        ...
        <div style="background:#0a0a1a;border:1px solid #003355;border-left:4px solid #4488ff;border-radius:10px;padding:1.25rem;margin-bottom:1rem;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;color:#ccc;font-size:.9rem;line-height:1.7;">
                <div>
                    ...
                    Данные: Kaggle — Corona Virus Report
                </div>
                <div>
        ...
        <div style="background:#0a0a1a;border:1px solid #003355;border-left:4px solid #4488ff;border-radius:10px;padding:1.25rem;margin-bottom:1rem;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;color:#ccc;font-size:.9rem;line-height:1.7;">
                <div>
                    ...
                    → γ = 1/12 ≈ <b>0.083</b><br><br>
                    Дереккөз: Kaggle — Corona Virus Report
                </div>
                <div>
...
- **CDC** — [Preparedness 101: Zombie Pandemic](https://stacks.cdc.gov/view/cdc/6023) *(официальный документ CDC 2011)*
- **CDC** — [Wikipedia: Preparedness 101](https://en.wikipedia.org/wiki/Preparedness_101:_Zombie_Apocalypse)
- **Pentagon CONPLAN 8888** — [Wikipedia](https://en.wikipedia.org/wiki/CONOP_8888)
- **Pentagon CONPLAN 8888** — [PDF документ Пентагона](https://www.stratcom.mil/portals/8/Documents/FOIA/CONPLAN_8888-11.pdf)
        ...
        glossary_data = {
            ...
            "KZ": [
                ...
                ("Model Explainability", "Модельді түсіндіру",
                 "Модель нәтижесін түсінікті форматта түсіндіру мүмкіндігі. зомби-апокалипсис ережелері — біздің explainability әдісіміз."),
            ],
        }
...
| OpenRouter API (GPT-OSS) | {t('tab_ai',lang)} |
...
**{'Maksat Omarov' if lang == 'EN' else 'Максат Омаров'}**
...
with tab6:
    ...
    disclaimer = {
        "RU": "⚠️ Это юмористический контент в стиле фильма «зомби-апокалипсис». Все советы вымышлены и служат для развлечения и демонстрации SIR-модели с точки зрения «другой стороны». Не кусайте людей в реальной жизни 🧟",
        "EN": "⚠️ Humorous content in the style of 'Survival Rules'. All tips are fictional. Do not bite people in real life 🧟",
        "KZ": "⚠️ Бұл «зомби-апокалипсис» стилінде юмористикалық мазмұн. Барлық кеңестер ойдан шығарылған. Нақты өмірде адамдарды тістемеңіз 🧟",
    }
